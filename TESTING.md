...
Use a tiny Paper plugin that depends on ZRestart and listens to the public API events.

Expected results:

- `Bukkit.getServicesManager().load(ZRestartApi.class)` returns a non-null service after ZRestart enables.
- `isRestartWithin(Duration.ofMinutes(10))` returns true when the active countdown is inside 10 minutes and false when it is farther away.
- `/zrestart now 10m Manual API test` fires `ZRestartCountdownStartedEvent` with `kind() == MANUAL`.
- A configured scheduled countdown fires `ZRestartCountdownStartedEvent` with `kind() == SCHEDULED`.
- `/zrestart stop` during an active countdown fires `ZRestartCountdownCancelledEvent`.
- `/zrestart reload` during an automatic scheduled countdown fires `ZRestartCountdownCancelledEvent` for the old countdown before any replacement scheduled start event.
- `/zrestart reload` during a manual countdown keeps the manual countdown active and does not fire a cancellation event.
- A countdown reaching zero fires `ZRestartExecutingEvent` only when pre-restart commands allow the restart to continue.
- With an optional dependency and ZRestart absent, the consuming plugin still loads if direct ZRestart imports are isolated behind a runtime bridge.
...
- [ ] ZRestart registers `ZRestartApi` through Bukkit services.
- [ ] Public API events fire for countdown start, cancellation, and execution.
